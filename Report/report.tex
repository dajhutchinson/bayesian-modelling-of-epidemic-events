...
\usepackage{amsmath,amsthm,amsfonts,bbm,caption,chngcntr,environ,fancyhdr,float,graphicx,hyperref,mdframed,natbib,subcaption,tikz}
...
\subsubsection*{Results - Experiment One}
  ...
  \begin{table}[H]
    ...
    \end{tabular}
    \caption{The Residual Sum of Squares Error (RSSE), 95\% confidence interval for $R_0,\beta,\gamma$ and the execution time of different ABC methods when given the full time-series and using the identity function as the summary statistic. Values are the mean of 50 independent runs of each algorithm.}
  ...
  \begin{figure}[H]
    ...
    \caption{Joint and marginal posterior distributions of $\beta$ and $\gamma$ parameters generated by different ABC-methods when provided with the full time-series of data.}
...
\subsubsection*{Results - Experiment Two}
  ...
  \begin{table}[H]
    ...
    \end{tabular}
    \caption{Leave-One-Out Cross-Validation Scores for different ABC algorithms when applied to the SIR model described in \textit{Figure \ref{fig_example_standard_sir_model}}. Each algorithm performed approximately 10,000 simulations each iteration, had rough tuning to achieve appropriate acceptance rates and used the identity function as the summary statistic.}
    \label{table_abc_methods_on_sir_model_loo_cv}
  \end{table}

  \begin{figure}[H]
    \centering\includegraphics[width=.7\textwidth]{Semi_Auto_ABC_SMC_full_data_CI.png}
    \caption{95\% Confidence Intervals for model fitted to the SIR model described in \textit{Figure \ref{fig_example_standard_sir_model}} using adaptive ABC-SMC with ~10,000 simulations and the identity function as the summary statistic. 95\% confidence interval for $R_0$ is [1.784,2.266].}
...
\subsection{Projection}\label{sec_projection}
  ...
  \begin{figure}[H]
    ...
    \caption{95\% CI when running adaptive ABC-SMC using summary statistics generated by semi-automatic ABC with only the first 10 or 20 days of data available. Accompanied by the generated posteriors.}
    \label{fig_semi_auto_restricted_data}
  \end{figure}

\subsection{Real Data}\label{sec_real_data}
