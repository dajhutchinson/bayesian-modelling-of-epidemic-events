...
\usepackage{amsmath,amsthm,amsfonts,bbm,caption,chngcntr,environ,fancyhdr,float,graphicx,hyperref,mdframed,natbib,tikz}
...
\section{ABC and Epidemic Events}\label{sec_abc_and_epidemic_events}
  ...
  \par In this section I pull the theory and methodology discussed in \textit{Sections \ref{sc_bayesian_modelling_and_epidemic_processes}-\ref{sec_summary_stats}} together and implement ABC methods to fit SIR models. I will first address which of the discussed ABC methods is the most suitable and shall then focus on summary statistic selection. For the choice of summary statistics we of course want to find, for computational efficiency, those which minimise dimensionality while also minimising loss in the fit. Additionally, we want to use summary statistics which are interpretable due to he use of these models by non-experts. % TODO rewrite
...
\par Summary statistic methods should perform well in all scenarios where ABC performs well as they are just extensions of ABC.
  ...
  \begin{enumerate}
    ...
    \item Brute force best set by calculating MSE for each set of SSs
  ...
  \begin{table}
    ...
    \begin{tabular}{|l|l|}
      \hline
      \textbf{Algorithm}&\textbf{LOO-CS Score}\\\hline\hline
      ``Best Samples'' ABC-Rejection Sampling&184,063\\\hline
      ABC-MCMC&90,713\\\hline
      ABC-SMC&19,300\\\hline
      Adaptive ABC-SMC&13,160\\\hline
    \end{tabular}
    \caption{Leave-One-Out Cross-Validation Scores for different ABC algorithms when applied to the SIR model described in \textit{Figure \ref{fig_example_standard_sir_model}}. Each algorithm performed approximately 10,000 simulations, had rough tuning to achieve appropriate acceptance rates and used the identity function as the summary statistic.}
    \label{table_abc_methods_on_sir_model}
  \end{table}

  \begin{figure}[H]
    \centering\includegraphics[width=.7\textwidth]{Semi_Auto_ABC_SMC_full_data_CI.png}
    \caption{95\% Confidence Intervals for model fitted to the SIR model described in \textit{Figure \ref{fig_example_standard_sir_model}} using adaptive ABC-SMC with ~10,000 simulations and the identity function as the summary statistic. Confidence Interval for $R_0$ is [1.871,2.170].}
