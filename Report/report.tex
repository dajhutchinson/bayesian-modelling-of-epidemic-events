...
\usepackage{amsmath,amsthm,amsfonts,bbm,chngcntr,environ,fancyhdr,float,graphicx,hyperref,mdframed,natbib,tikz}
...
\subsection*{Standard SIR Model}
  ...
  \par These two interactions are governed by the following three differential equations which represent the change in the total number of individuals in each compartment over time \cite[]{application_of_sir_epidemiological_model}.
  ...
  \par In \textit{Figure \ref{fig_example_standard_sir_model}} we can observed that there is a point at which the size of the infectious population begins to decrease. We can mathematically identify this time-point where the gradient $\frac{dI}{dt}$ becomes negative. As $\frac{dI}{dt}=I(\beta S-\gamma)$ and $I\geq0$ at all time-points, the gradient becomes negative when $\beta S-\gamma<0$. Thus the maximum number of infections occur at time-period $t$ where (\ref{eqn_sir_tipping_point_susceptible}) is satisfied.
...
\subsection*{SIR Model with demography}
  ...
  \begin{figure}[H]
    ...
    \label{fig_sir_model_diagram_w_vital_dynamics}
  \end{figure}

  \par Two new parameters are introduced to this model: $\lambda$ the number of births per time-period; and $\mu$ the number of natural deaths per time-period. It is assumed that an individual is equally likely to die naturally, regardless of which compartment they are in. The reciprocal of the death rate $1/\mu$ is the average life-span of each individual in the population. is Both $\lambda$ and $\mu$ are independent of the total population size. The following three differential equations model all of these interactions \cite[]{application_of_sir_epidemiological_model}:
  ...
  \par This system of equations is very similar to that of the SIR model without demography except each equation has a term subtracts the number of natural deaths each time-period, which is proportional to the current population size of the associated compartment. And, the equation for the changes in the susceptible population has a term for introducing natural births.
  ...
  \begin{box_remark}[Equilibria of SIR model with Demography]\label{rem_equilibria_sir_w_demography}
    ...
    \begin{itemize}
      ...
      \[\begin{array}{rrcl}
        &\frac{dI}{dt}&=&0\\
        \implies&\beta S^*I^*-\gamma I^*-\mu I^*&=&0\\
        \implies&\beta S^*-\gamma-\mu&=&0\\
        \implies&S^*&=&\frac{\mu+\gamma}\beta\\
        &&=&\frac1{R_0}\\
        \\
        &\frac{dS}{dt}&=&0\\
        \implies&\lambda-\beta S^*I^*-\mu S^*&=&0\\
        \implies&\lambda-\frac{\beta I+\mu}{R_0}&=&0\\
        \implies&I^*&=&\frac{\lambda R_0-\mu}\beta\\
        \\
        &\frac{dR}{dt}&=&0\\
        \implies&\gamma I^*-\mu^*&=&0\\
        \implies&R^*&=&\frac\gamma\mu I^*\\
        &&=&\frac{\gamma\lambda}{\mu+\gamma}-\frac\gamma\beta
      \end{array}\]
      Thus, when the $R_0\geq1$ an equilibrium is reached when the compartment populations fulfil
      \[ (S,I,R)=\left(\frac1{R_0},\ \frac{\lambda R_0-\mu}\beta,\ \frac{\gamma\lambda}{\mu+\gamma}-\frac\gamma\beta\right) \]
...
\subsection*{Other Compartmental Models}
  ...
  %TODO Compartment for different ages georgraphy (network models are better)

\subsection*{Extensions to SIR Models}

  % TODO parameters drawn from a distribution. (Not as necessary for large populations (esp when large for each compartment), )
  % TODO more compartments for different ages
  % TODO parameters vary over time
  % TODO other dynamics (Vaccinations, people loose immunity)
...
\subsection{ABC for Model Choice}\label{sec_abc_model_choice}
  ...
  \begin{box_algorithm}[ABC Model Selection ``Rejection Sampling'']\label{alg_abc_mc}
...
\section{ABC and Epidemic Events}\label{sec_abc_and_epidemic_events}
  ...
  \begin{enumerate}
    \item Use standard SIR model, SIR model with distribution for $\beta,\gamma$.
    \item Choose loads of hand-crafted summary statistics.
    \item Use approximate sufficiency; minimising entropy; and two-step ME to choose best set of hand-crafted (Compare with leave-one-out cross validation).
    \item Plot dimensionality of summary stats against MSE (use ME to choose best at each dimensionality)
    \item Use Semi-Auto to generate summary stats (compare to best subsets from previous).
    \item Stick in some covid data (e.g. active cases of first wave from italy). Note that we are ignoring changes in lock down.
    \item SIR model with stepped $\beta,\gamma$ wrt time
  \end{enumerate}
