...
\section{ABC and Epidemic Events}\label{sec_abc_and_epidemic_events}
  ...
  \par In this section I pull the theory and methodology discussed in \textit{Sections \ref{sc_bayesian_modelling_and_epidemic_processes}-\ref{sec_summary_stats}} together and implement ABC methods to fit SIR models. I will first address which of the discussed ABC methods is the most suitable and shall then focus on summary statistic selection. For the choice of summary statistics we of course want to find, for computational efficency, those which minimise dimensionality while also minimising loss in the fit. Additionally, we want to use summary statistics which are interpretable due to he use of these models by non-experts. % TODO rewrite

  \par A common difficulty in modelling epidemic events is the lack of available data. Morever, most epidemics are only experienced once making it very difficult to account for random variabiltiy. % TODO
