...
\subsection{Properties of Summary Statistics}\label{sec_properties_of_summary_statistics}
  ...
  \par When evaluating a summary statistic for use in ABC there are several properties, both mathematical and practical, to consider. I first discuss practical properties to consider, and then the key mathematical property of sufficiency.
...
\subsubsection*{Sufficiency}\label{sec_sufficiency}
  ...
  \par \textbf{Theorem \ref{the_sufficiency_of_superset}} states that if we have a set of summary statistics which are sufficient for a set of parameters, then adding more summary statistics will never increase (or decrease) the amount of relevant information being extracted from the sampled data.
...
\subsubsection{Approximate Sufficient Subset}\label{sec_approximate_sufficient_subset}
  In \cite[]{Approximately_sufficient_statistics_and_bayesian_computation} a sequential process is proposed for selecting a subset of summary statistics from a larger set of hand-crafted statistics, in such a way that ABC methods perform approximately as well when using the subset compared to using the superset.

  \par For the theoretical basis of their algorithm, Joyce \& Marjoram define the concepts of score and $\varepsilon$-approximate sufficiency. This are used to determine how extra information each additional summary statistic extracts.

  \begin{box_definition}[Score $\delta_k$]\label{def_score}
    % TODO
    \cite[]{Approximately_sufficient_statistics_and_bayesian_computation}
  \end{box_definition}

  \begin{box_definition}[$\varepsilon$-Approximate Suffiency]\label{def_approximate_suffiency}
    % TODO
    \cite[]{Approximately_sufficient_statistics_and_bayesian_computation}
  \end{box_definition}

  \begin{box_algorithm}[Select an Approximately Sufficient Subset of Summary Statistics - Theoretical]
    \begin{algorithm}[H]
  ...
  \begin{box_algorithm}[Select an Approximately Sufficient Subset of Summary Statistics - Practical]
    \begin{algorithm}[H]
      \SetKwInOut{Require}{require}
      \Require{Set of summary statistics $S$}
      $S'\leftarrow\emptyset$\\
  ...
  \begin{box_algorithm}[Evaluate Proposed Statistic]
    \begin{algorithm}[H]
